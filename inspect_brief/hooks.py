...
class InspectBriefHooks(Hooks):
    ...
    def __init__(self) -> None:
        self._run_summaries: dict[str, tuple[int, int, int, int, str]] = {}
    ...
    async def on_task_end(self, data: TaskEnd) -> None:
        ...
        task_count, failed_task_count, export_error_count, result_count, _ = (
            self._run_summaries.get(data.run_id, (0, 0, 0, 0, csv_path))
        )
        self._run_summaries[data.run_id] = (
            task_count + 1,
            failed_task_count + (data.log.status != "success"),
            export_error_count + export_failed,
    ...
    async def on_run_end(self, data: RunEnd) -> None:
        ...
        """
        task_count, failed_task_count, export_error_count, row_count, csv_path = (
            self._run_summaries.pop(data.run_id, (0, 0, 0, 0, "N/A"))
        )
        if data.exception is not None:
            logger.error(
                "[inspect-brief] summary tasks=%s failed=%s export_errors=%s "
                "rows=%s csv: %s run_exception=%r",
                task_count,
                failed_task_count,
                export_error_count,
                row_count,
